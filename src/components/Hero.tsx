...
export default function AspectRatioFixedHero() {
  ...
  useEffect(() => {
    ...
    const starMaterial = new THREE.PointsMaterial({
      size: 2.5, color: 0xffffff, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false
    });
    const starField = new THREE.Points(starGeometry, starMaterial);
    scene.add(starField);

    // --- ASTRONAUTA CON ASPECT RATIO CORREGIDO ---
    const textureLoader = new THREE.TextureLoader();
    const astronautTexture = textureLoader.load('/images/astronaut-final-v2.png', (tex) => {
      // Calculamos la proporción real de la imagen
      ...
      const updateLayout = () => {
        ...
        // Aplicamos la proporción a la escala X para que NO SE ESTIRE
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-[#000000] pt-24 pb-12 px-6">
      <div ref={mountRef} className="absolute inset-0 z-0 pointer-events-none" />
      
      {/* Scrim oscuro dinámico para legibilidad */}
      <div className="absolute inset-0 z-1 pointer-events-none bg-[radial-gradient(circle_at_30%_50%,rgba(0,0,0,0.8)_0%,transparent_60%)] md:bg-[radial-gradient(circle_at_20%_50%,rgba(0,0,0,0.7)_0%,transparent_50%)]" />
        ...
        <motion.div style={{ opacity }} className="max-w-3xl flex flex-col items-center md:items-start">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6">
            <span className="inline-block px-4 py-1.5 rounded-full border border-cyan-meta/30 bg-cyan-meta/10 text-cyan-meta text-[10px] font-mono tracking-[0.3em] uppercase backdrop-blur-md">
              Paid Media | Master of Scaling
            </span>
          </motion.div>

          <motion.h1 
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            className="text-5xl md:text-8xl font-black text-white leading-tight tracking-tighter mb-8 drop-shadow-[0_10px_30px_rgba(0,0,0,0.8)]"
          >
          ...
          <motion.p 
            ...
            className="text-lg md:text-xl text-gray-200 font-light leading-relaxed mb-12 max-w-xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.9)]"
          >
          ...
          >
            ...
            <button className="w-full sm:w-auto px-8 py-4 glass rounded-xl text-white font-bold text-base hover:bg-white/10 transition-colors uppercase tracking-widest text-[10px]">
              Ver Resultados
            </button>
          </motion.div>
        </motion.div>
